...
from Crypto.Cipher.AES import MODE_ECB, block_size
...
class Crypto():
    ...
    @staticmethod
    def PadRemain(data,blocksize=AES.block_size):
    ...
    @staticmethod
    def UnpadRemain(data):
        # The reverse funtion of `padRemain`
        pad = data[-1]
    ...
    @staticmethod
    def AESEncrypt(
        data, key, mode=AES.MODE_ECB , iv='',padder='PadRemain'        
    ):
        ...
        padder = getattr(Crypto,padder)
        encrypt_aes = encryptor.encrypt(str.encode(padder(data)))
        return encrypt_aes

    @staticmethod
    def AESDecrypt(
        data,key,mode=AES.MODE_ECB,iv='',unpadder='UnpadRemain'
    ):
        ...
        unpadder = getattr(Crypto,unpadder)
    ...
    @staticmethod
    def WeapiCrypto(params, aes_key2='mos9527ItoooItop', encSecKey=''):
        '''Used in web & PC client'''
        params = str(params)
        # 1st go,encrypt the text with aes_key and aes_iv
        params = str(base64.encodebytes(Crypto.AESEncrypt(
            data=params, key=Crypto.weapi_aes_key, iv=Crypto.weapi_aes_iv,mode=AES.MODE_CBC)), encoding='utf-8')
        # 2nd go,encrypt the ENCRYPTED text again,with the 2nd key and aes_iv
        params = str(base64.encodebytes(Crypto.AESEncrypt(
            data=params, key=aes_key2, iv=Crypto.weapi_aes_iv,mode=AES.MODE_CBC)), encoding='utf-8')
        # 3rd go,generate RSA encrypted encSecKey
        if not encSecKey:
            # try not to generate this on-the-fly as it's quite slow
            encSecKey = hex(Crypto.RSAEncrypt(aes_key2, Crypto.weapi_rsa_pubkey))[2:].zfill(256)
        return {
            'params': params,
            'encSecKey': encSecKey
        }
    ...
    def LinuxCrypto(params):
        ...
        return {
            'eparams':''.join([hex(b)[2:] for b in Crypto.AESEncrypt(params,key=Crypto.linuxapi_aes_key,mode=AES.MODE_ECB)])
        }
    ...
    def EapiCrypto(url,params):
        ...
        digest = Crypto.HashDigest(Crypto.eapi_digest_salt % {'url':url,'text':params})
        params = Crypto.eapi_data_salt % ({'url':url,'text':params,'digest':digest})
        return {
            'params':''.join([hex(b)[2:] for b in Crypto.AESEncrypt(params,key=Crypto.eapi_aes_key,mode=AES.MODE_ECB)])
        }
    ...
    def EapiDecrypt(cipher):
        ...
        return Crypto.AESDecrypt(cipher,Crypto.eapi_aes_key,mode=AES.MODE_ECB)
    # endregion