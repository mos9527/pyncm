...
from . import HashHexDigest, HexCompose, HexDigest, RandomString, security
from .aes import AES
...
def RSAEncrypt(data: str, n, e, reverse=True):
    ...
    r = pow(m, e, N)
    return HexCompose(f"{r:x}".zfill(256))
...
def WeapiEncrypt(params, aes_key2=None):
    ...
    params = str(
        base64.encodebytes(
            AESEncrypt(data=params, key=aes_key2, iv=WEAPI_AES_IV, mode=AES.MODE_CBC)
        ),
        encoding="utf-8",
    )
...
def LinuxApiEncrypt(params):
    ...
    return {
        "eparams": HexDigest(
            AESEncrypt(params, key=LINUXAPI_AES_KEY, mode=AES.MODE_ECB)
        )
    }
