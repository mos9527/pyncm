...
WEAPI_ABROAD_IV = ([ord(s) for s in WEAPI_ABROAD_XOR_KEY] * 5)[:64]
...
def wm_hex_to_ints_hb(e):
    ...
    return c    
def wm_permutate_array_Qb(e, c, d=0, r=WEAPI_WATCHMAN_SBOX, g=WEAPI_WATCHMAN_PAD):
    # dynamic P-box w/ another array
    h, m,g = 0,[],str(g)
    ...
    if d == 1:
        ...
        return None
    return ''.join(m)
def wm_double_sbox_Sb(e, c=[], d=WEAPI_WATCHMAN_PAD):
    # dynamic s-box
    r = 3
    h = 0
    g = []
    while (h < len(e)):
        if (h + r <= len(e)):
            g.append(wm_permutate_array_Qb(e, h, r, c, d))
            h += r
        else:
            g.append(wm_permutate_array_Qb(e, h, len(e) - h, c, d));
            break
    return "".join(g)
def wm_map_string_Tb(a):
    return wm_double_sbox_Sb(a,BASE64,'=')
def wm_generate_OTP_b():
    ...
    c = [ 0 ] * 8    
    wm_permutate_ga(e, 0, c, 0, 4);
    wm_permutate_ga(d, 0, c, 4, 4);  
