...
# region secrets
WEAPI_WATCHMAN_MAP = 'LPc4uQNptC2A6y.R90DOBIroS+qnx/eb3FM8fW1UZG7VmwvksgjhaTKzlXdiYEHJ'
...
WEAPI_ABROAD_XOR_KEY = 'fuck~#$%^&*(458' # ookay...
...
WEAPI_ABROAD_SBOX_KEY_1 = ([ord(s) for s in WEAPI_ABROAD_XOR_KEY] * 5)[:64]
# sbox initalization vector
# endregion
...
def normalize(e):
    # (-inf,inf) -> [-127,127]
    if e < -128:
        return normalize(128 - (-128 - e))
    if (e >= -128 and e <= 127):
        return e
    if (e > 127):
        return normalize(-129 + e - 127)        
def normalize_to_bytes(b):
    '''Integer to bytes - int(32 bits) -> [4 bytes]'''
    return [normalize(b >> 24 & 255),normalize(b >> 16 & 255),normalize(b >> 8 & 255),normalize(b & 255)]
# endregion 
...
def wm_xor_cipher_La(e):    
    ...
    for q in range(0,len(e)):         
        g.append(normalize(jxor(e[q],d[q % len(d)])))
        g[q] = normalize(-g[q])
    return to_hex_string(g)
def wm_suffler_ga(e, c, d, r, g):
    '''Array shuffler'''
...
def wm_hex_to_ints_hb(e):
    ...
    for r in range(0,len(e),2):
        h = jls(int(e[r], 16),4)
        m = int(e[r + 1], 16)
        c.append(normalize(h + m))
    return c    
def wm_obfuscate_array_Qb(e, c, d=0, r=WEAPI_WATCHMAN_MAP, g=WEAPI_WATCHMAN_PAD):
    ...
    if d == 1:
        ...
        return None
    return ''.join(m)
def wm_double_mapping_Sb(e, c=[], d=WEAPI_WATCHMAN_PAD):
    ...
    while (h < len(e)):
        if (h + r <= len(e)):
            g.append(wm_obfuscate_array_Qb(e, h, r, c, d))
            h += r
        else:
            g.append(wm_obfuscate_array_Qb(e, h, len(e) - h, c, d));
            break
    return "".join(g)
def wm_map_string_Tb(a):
    return wm_double_mapping_Sb(a,BASE64,'=')
def wm_generate_OTP_b():
    # Tokenzies current time
    e = int(time() * 1000) 
    c = math.floor(e / 4294967296)
    d = e % 4294967296
    e = normalize_to_bytes(c)
    d = normalize_to_bytes(d)
    c = [ 0 ] * 8    
    wm_suffler_ga(e, 0, c, 0, 4);
    wm_suffler_ga(d, 0, c, 4, 4);  
    ...
    return c
def wm_generate_config_chiper_bc(c,f=1,g=WM_DID):
...
def c_neg_xor(v1,v2): return jxor(normalize(v1),normalize(v2))
def c_map_with_magic(src,map):        
...
def c_decrypt_abroad_message(hexstring):
    ...
    result = []
    boxA = WEAPI_ABROAD_SBOX_KEY_1
    for i in range(0,len(source),64):
        box = source[i:i+64]
        boxB = c_map_with_magic(c_map_with_magic(box,WEAPI_ABROAD_MAP),WEAPI_ABROAD_MAP)
        boxC = [c_neg_xor(boxB[i],boxA[i]) for i in range(0,64)]    
        boxD = [normalize(boxC[i] + normalize(-boxA[i])) for i in range(0,64)]    
        boxE = [c_neg_xor(boxD[i],WEAPI_ABROAD_SBOX_KEY_1[i]) for i in range(0,64)]                
